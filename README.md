...
- **الحضور والانصراف:** كود QR لكل فرع (بيتولّد من لوحة المسؤول) بيودّي لصفحة `attendance.html` منفصلة، مفتوحة للكل بدون تسجيل دخول (زي فورم الصيادلة/العمال). أي حد يعرف رقم سجل حضور بعينه (idNumber+تاريخ) يقدر يقرأه مباشرة (`get`) — ده ضروري عشان صفحة الحضور تتأكد إن الشخص سجّل حضور قبل ما يسجّل انصراف، لكن **تصفح كل السجلات مع بعض (`list`)** مقصور على المسؤول أو صاحب الفرع بس. الكتابة مفتوحة للجميع (زي باقي الفورمات العامة)، والحذف ممنوع نهائيًا حفاظًا على السجل.

---

## تحديث: نظام الحضور والانصراف

### الفكرة
- من **admin.html**، كارت "كود QR للحضور والانصراف": اكتب رقم الفرع ودوس "توليد / عرض الكود"، هيطلعلك صورة QR ولينك — اطبع الصورة وحطها في الفرع.
- أي صيدلي/عامل يصوّر الكود بموبايله، هيوصله لصفحة `attendance.html` مقفولة على رقم الفرع ده تلقائي (من كود سري مش قابل للتخمين).
- يكتب رقم هويته واسمه، ويدوس "تسجيل حضور" أو "تسجيل انصراف". الوقت بييجي من سيرفر Firebase نفسه، والموقع الجغرافي بيتسجل تلقائي لو المتصفح سمح بيه.
- **المسؤول** يشوف تقرير شامل لكل الفروع في تبويب "الحضور والانصراف" في `admin.html`، مع فلترة وتصدير Excel.
- **كل فرع** يقدر يشوف سجل حضور فريقه هو بس من تبويب "الحضور والانصراف" في `branch.html` (عرض فقط، مفيش تسجيل من هنا).

### ملاحظة أمان مهمة (تزوير الحضور)
الطريقة دي بتثبت إن الشخص كان **قريب فعليًا من الفرع** (محتاج يشوف الكود المطبوع بعينه)، لكنها **مش بتثبت مين بالظبط ضغط الزرار** — لو زميلين واقفين مع بعض، حد يقدر يسجّل لزميله. الحل الأقوى 100% كان حساب شخصي منفصل بباسورد لكل صيدلي وعامل، لكن ده أصعب في الإدارة. لو حبيت لاحقًا نرفع مستوى الحماية، ممكن نضيف رمز PIN شخصي لكل فرد.
...
لو الفرع عنده إحداثيات محفوظة، وحد سجّل حضور أو انصراف من مكان بعيد عن الفرع (أكتر من 100 متر)، النظام **مش بيمنعه** — بيسجّل العملية عادي، بس بيحط علامة 📍 حمرا جنب الوقت في تقرير المسؤول والفرع، عشان المسؤول يراجعها لو حبّ.
